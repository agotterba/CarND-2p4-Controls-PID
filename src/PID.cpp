...
void PID::Init(std::string Name, double Kp, double Ki, double Kd) {
  ...
  ki_ = Ki;
  kd_ = Kd;
  //current_cte_ = 0.0; //set in reset instead
  //prev_cte_ = 0.0; //set in reset instead
  //delta_cte_ = 0.0; //set in reset instead
  int_cte_ = 0.0;
...
void PID::UpdateError(double cte) {
  ...
  delta_cte_ = current_cte_ - prev_cte_;
...
double PID::TotalError() {
  ...
  //limit return to between -1 and +1
  te = te >  1 ?  1 : te;
  te = te < -1 ? -1 : te;
